...
[Astral](https://astral.sh)'s uv + ruff + ty.
[`docs/development.md`](docs/development.md) has the repo layout, the commands
to run before opening a PR, and the testing model.

## Reporting a bug or asking for a feature

Open an issue from the Issues tab: Bug report for something the engine gets
wrong, Feature request for something it cannot do. For a security vulnerability
use the Security tab instead, not an issue (see [SECURITY.md](SECURITY.md)).
...
- **Flavor-specific needs belong in shipmate.** They land as an action input or
  feature, never as patch code in a consuming repo.
