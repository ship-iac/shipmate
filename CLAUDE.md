...
- **Post-merge detect (exact-plan; PRD-2 Deviation A):** `deploy.yml`'s `deploy-detect` maps the merge commit → its PR head SHA and orders the reviewed `.otplan` artifacts whose `apply` check is still **pending** into waves; completed checks (pre-merge-applied via PRD 3, or no-op) are skipped → idempotent no-op. The pending apply checks ARE the work queue — **no "last deployed ref" diff** (Terramate has no deployment memory; the marker was only ever a substitute for this queue). A GHA-superseded deploy leaves its stacks pending+visible, recovered by re-running that deploy (Deviation B).
