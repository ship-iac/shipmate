...
shipmate is a **serverless [Terrateam](https://docs.terrateam.io)**: the same
...
- **`deploy.yml`** (`on: push main`) is the **exact-plan apply** path.
  ...
  (pre-merge via PRD 3, or a no-change re-plan) has a completed check → deploy
  **no-ops** it.
...
(`mate apply`) is delivered in PRD 3 (it needs the GitHub App to update
checks).
