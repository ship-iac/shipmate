...
Breaking a lock whose holder is still running corrupts state, so every guard here is
load-bearing:

- the action carries no `tofu apply` and no apply-completion step, checked over the whole file
  text so "unlock cannot apply" is structural rather than intentional;
- its three `terramate run` lines are compared, in order and as raw lines, against hand-written
  constants. Raw, not shlex tokens, because the quoting of `"$LOCK_ID"` is part of the contract,
  reaching a `force-unlock` argv, and shlex would drop it;
- the release step runs only on the probe's `held == 'true'` output, the whole expression
  compared to a constant;
- `$STACK` and `$LOCK_ID` reach the shell through `env:`, never `${{ }}`.

Two guards execute the shipped shell bodies rather than reading them: the report's no-lock and
could-not-determine states, which must never collapse into one message and of which only the
first succeeds, and the conditional rendering of `lock_created` and `lock_operation`, which
`scripts/lock-info` may legitimately return empty.

Every constant here is hand-written and never derived from action.yml: an expectation read back
out of the file under test passes whatever that file says.
"""
...
#: Every `terramate run` the cell is expected to make, in order. Whole lines, because a window
#: inside them asserts nothing about either edge and the flags at both edges are the load-bearing
#: part: `--disable-safeguards=git-out-of-sync` on the left, `-force "$LOCK_ID"` on the right.
_EXPECTED_RUNS = [
...
#: The release step's gate, whole. `-force` in `_EXPECTED_RUNS` and this gate are the pair that
...
def _live_lines():
    ...
    Every step with a `run:`, not only `shell: bash` ones: selecting on the shell would let a
    command move to `shell: sh` and run unexamined. Comments go first, because a mention in prose
    is not an invocation, then backslash joins, so a command split across lines is matched as the
    one command it is.
    """
...
def test_the_unlock_cell_can_never_apply():
    """Whole file text, comments and description included: the absence has to be structural, so
    even a commented-out apply reds rather than sitting there as the next refactor's starting
    point."""
...
def test_the_cell_runs_exactly_these_three_terramate_invocations():
    """One selector, one comparison. Comparing the whole list also pins the count and the
    init-probe-release order without either being a second, disagreeable check."""
...
def test_no_step_swallows_its_own_failure():
    """Every step here fails the cell it belongs to. `continue-on-error` on the init in
    particular would turn "the engine could not look at the lock" into a green cell carrying an
    annotation nobody reads across dozens of cells."""
...
def _run_body(tmp_path, step_id, env, *, terramate_body="return 0"):
    """Execute the real, unmodified `run:` body with `terramate` replaced by a bash function.
    Bash resolves a function before PATH, so this needs no fake executable or exec bit, which is
    fragile on a Windows dev box.

    The stub `return`s rather than `exit`s: the release step calls terramate as a plain command in
...
def test_the_cell_takes_no_credential_or_artifact_inputs():
    ...
    `config-mode` is the exception every cell action takes -- it selects how the cell resolves
    its identity variables and reaches no credential."""
