...
eventually, so neither side is derived from the other and the two are never
compared to each other -- `APPLY_ENV` pins the YAML whole, `BINDINGS` states the
rule's outcome for the cases that distinguish it, and the script is compared to
that.
...
#: the YAML expression, never generated by running the script.
BINDINGS = {
...
#: snapshot's steps, in order, by action path with the SHA dropped -- a repin
#: must not redden this, a reorder must.
SNAPSHOT_STEPS = [
