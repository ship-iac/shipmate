...
def test_verification_sits_between_the_download_and_the_decrypt():
    # Not merely "before the apply": after the decrypt it would already have spent a decrypt,
    # and after restore-state a state restore, on a plan of another tree.
...
def test_the_step_is_attributable_in_the_cell_summary():
    ...
    # partial check would not pin that.
...
def _run_step(tmp_path, *, record=None, observed=_PLANNED):
    """Execute the real, unmodified step script from action.yml with `git` replaced by a bash
    function. Bash resolves a function before searching PATH, so this needs no fake executables.
    `record` is the content of planned-head.txt in the working directory, or None to leave it
    absent.
...
def test_a_record_disagreeing_with_head_aborts_naming_both_commits(tmp_path):
    ...
    out = r.stdout + r.stderr
    assert "::error::" in out
    # Both commits, so the reader can tell which plan and which checkout. One of them alone is
    # not actionable.
...
def test_a_matching_record_proceeds_and_leaves_no_file_in_the_checkout(tmp_path):
    ...
    assert r.returncode == 0, f"stdout={r.stdout!r} stderr={r.stderr!r}"
    # Moved, not copied: the action runs in the consumer's checkout and must not leave a stray
