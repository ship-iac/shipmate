...
`SHIPMATE_APP_ID` read, the check-run shape) lives here rather than once per
detect -- three copies drift one at a time, and the one left stubbing the old
shape keeps passing against output `gh` no longer produces.
"""
...
def completed_names(apply_detect, monkeypatch, checks, app_id=APP_ID):
    ...
    runs it. A raw string in `checks` is passed through unparsed, for the
    malformed-line case.
    """
    ...
    return apply_detect.completed_apply_names("acme/repo", HEAD)
