...
The working-tree reader is the behaviour test_internal_pins.py relies on. The commit reader is
what pin_status needs: pins as they were at a historical commit, not as they are on disk now.
"""

import pinrefs
import pytest

#: Real history fixture for test_refs_at_commit_reads_that_commits_tree_not_disk. In a shallow
#: clone this commit is absent, refs_at() on it returns [], and that test passes vacuously
#: ({} != now instead of a real diff). A bare module-level `assert` would raise at import time and
#: abort collection of this whole module, so a shallow checkout would turn an unrelated `scripts/`
#: edit's test run red; skip loudly instead. CI checks out with fetch-depth: 0, where this
...
def test_source_paths_honours_an_alternate_root(tmp_path):
    # The re-pin tools reuse this against a fixture directory rather than carrying a second copy
    # of the glob.
    (tmp_path / ".github" / "workflows").mkdir(parents=True)
    (tmp_path / ".github" / "workflows" / "plan.yml").write_text("", encoding="utf-8")
    # .yaml is equally valid to GitHub, and a workflow escaping the guard by spelling its
    # extension differently is the silent hole this model prevents.
...
def test_refs_at_commit_reads_that_commits_tree_not_disk():
    # 83b37bb is a convergence commit whose apply-env-level.yml pin differs from today's.
    # Reading it must yield that commit's SHA, not the working tree's.
...
def test_source_paths_working_tree_read_does_not_call_git(monkeypatch):
    # Only the commit-tree branch calls ls-tree. A working-tree read (commit=None) must not
    # touch git at all, so the GitFailure guard cannot change its behaviour.
    def fail(*_a):
