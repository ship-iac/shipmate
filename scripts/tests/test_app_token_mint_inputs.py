...
**Before bumping the `actions/create-github-app-token` pin:** what makes the rename safe is
upstream's internal resolution, not its documented contract. `client-id` is described upstream as
the App's Client ID (`Iv23li…`), and the engine supplies the numeric App ID under that key.
Upstream could add a format check on `client-id`, or resolve `app-id` first, without considering
it a breaking change, and every mint in the engine would fail at once: no `shipmate / gate`
status, so every pull request is unmergeable, no apply checks, `shipmate apply` and `doctor`
silent. These tests cannot catch that -- they read the engine's YAML, not the pinned action's
code, and the deprecation warning that used to be the early signal is gone because of this
change. On any bump, re-read the new version's input resolution and confirm the value still
...
#: The App-identity expressions the engine legitimately threads into a mint: a composite action
#: reads its own `app-id` input, a workflow reads the repository variable consumers set. Pinned
#: as a closed set, so a mint cannot quietly start identifying the App from some other,
#: unreviewed source.
...
def _engine_yaml():
    ...
    Deliberately not a narrow `actions/*/action.yml` plus `.github/workflows/*.yml` pair. GitHub
    accepts a mint in shapes those globs miss -- `action.yaml`, a nested
    `actions/<a>/<b>/action.yml`, `.github/actions/<a>/action.yml`, a `.yaml`-spelled workflow --
    and a guard that silently skips a file is worse than no guard, because it reads as coverage.
    Sweeping every YAML in both trees costs nothing: a file with no mint step contributes
    nothing.
    """
...
def _mint_steps():
    ...
    Reads both shapes out of each document, a composite action's `runs.steps` and a workflow's
    `jobs.<name>.steps`, so which shape a mint lives in never decides whether it is checked.
    """
    found = []
...
def test_the_engine_has_mint_sites_to_check():
    """A refactor that moved every mint somewhere this sweep cannot see would otherwise make
    test_no_mint_passes_the_deprecated_app_id_key and
    test_every_mint_identifies_the_app_with_client_id pass vacuously.

    Asserts non-emptiness only. A count floor would also red when an action legitimately stops
    needing an App token, under a message about the sweep being broken: a false diagnosis, and
    the wrong test to edit in that pull request.
    """
