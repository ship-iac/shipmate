...
how the engine reaches the App key. The consumer's `plan` job runs on
...
- **In shared mode the plan path holds the apply role.** The bare `<env>`
  ...
  drift jobs, and a job that omits it is rejected at load with no job and no
...
- **Fork pull request workflows.** Require approval for all outside
  ...
  the reason shipmate's own `shipmate.yml` may use `pull_request_target` while a
  labeler workflow may not. `shipmate doctor` names any *other* workflow file
  declaring the trigger; `shipmate.yml` is exempt by exact name.
...
table's variable references. comment-ops and the plan `summary` job bind
`shipmate-engine`, so that environment's variables reach them as well.
...
`actions/comment-ops` refuses a commented `shipmate plan` naming a fork's pull
request, rather than dispatching a run whose refusal would land on the dispatch
ref. **That is a message, not a third layer.** It reads the same head repository
the two guards above read, and a head it cannot read is dispatched anyway, so
nothing keeping a fork out of a plan cell depends on it.
