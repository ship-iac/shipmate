...
| 19 | `id-token: write` on the call-site job of every consumer wrapper that calls the engine's `apply.yml`, `unlock.yml` and `deploy.yml` — **not** `plan.yml` | Consumer workflow YAML | Nothing — it is **required**: GitHub caps a called workflow's permissions at each `uses:` boundary, so without it every apply and unlock run fails at workflow-resolution time, cloud or not |
